...
ARCHITECTURE RTL OF GPIO IS
    CONSTANT c_GPIO_DATADIR_REG_INDEX : INTEGER := 0;
...
BEGIN    
    ...
    io_Pin_Port <= r_Pin_Port_Out;    
        
    -- Pin interface (Controller <-> Pin)
    PROCESS(i_Rst, a_DATAOUT_reg, a_DATADIR_reg)
    BEGIN
        IF(i_Rst = '1') THEN
            r_Pin_Port_Out <= (OTHERS => 'Z');
        ELSE
            loop_OUTPUT_PIN_INTERFACE:
            FOR i IN 0 TO io_Pin_Port'LENGTH - 1 LOOP
                IF(a_DATADIR_reg(i) = '1') THEN
                    r_Pin_Port_Out(i) <= a_DATAOUT_reg(i);
                ELSE
                    r_Pin_Port_Out(i) <= 'Z';
                END IF;       
